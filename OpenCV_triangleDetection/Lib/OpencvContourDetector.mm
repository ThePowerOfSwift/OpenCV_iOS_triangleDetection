...
-(void)findBiggestPolygon:(int)_nbVerticies
{
    ...
    while(contours)
    {
        contourPoints = cvApproxPoly(contours, sizeof(CvContour), contoursStorage, CV_POLY_APPROX_DP, cvContourPerimeter(contours)* EPS_COEFFICIENT, 0);

        if(contourPoints->total == _nbVerticies)
        {
            area = cvContourArea(contourPoints);

            if(area > biggestArea)
            {
                biggestPolygon = contourPoints;
                biggestArea = area;
